...
use nom::{named, exact, none_of, many0_count, many_till, recognize, eof, pair, char};

named!(
    parse_line<&str, (usize, &str)>,
    exact!(pair!(
        many0_count!(char!(' ')),
        recognize!(many_till!(none_of!(""), eof!()))
    ))
);
    ...
    for line in BufReader::new(s.as_bytes()).lines() {
        if let Ok(line) = line {
            if let Ok((_, (sp, content))) = parse_line(&line) {
                ...
                let mut splited_keys = keys
                    .iter()
                    .flat_map(|x| x.1.split(':').next());
